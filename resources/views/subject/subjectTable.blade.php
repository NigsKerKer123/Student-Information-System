      ...
      <table class="items-center bg-transparent w-full border-collapse ">
        ...
        @foreach($subject as $subjectData)
          <tr>
            ...
            <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
              <!-- edit button -->
              <button data-modal-target="Edit-modal{{$subjectData->subjectid}}" data-modal-toggle="Edit-modal{{$subjectData->subjectid}}" class="inline-flex items-center px-4 py-2 text-white text-sm font-medium rounded-md" type="button" style="background-color: darkorange; transition: background-color 0.3s ease-in-out;"
                onmouseover="this.style.backgroundColor='gray'"
                onmouseout="this.style.backgroundColor='darkorange'">
                EDIT
              </button>

              <form method="POST" action="{{route('subjectDelete', $subjectData->subjectid)}}" style="display: inline-block;">
              @csrf
              @method('DELETE')
